...
ask.lua : an example script with help text and a test suite
(c)2022, Tim Menzies <timm@ieee.org>, BSD-2 

Reads a csv file with attribute names on row1, example names in column1,
and all else are numbers or "?".

USAGE:   ask.lua  [OPTIONS]

OPTIONS:
  -d  --dump  on crash, dump stack = false
  -f  --file  csv file             = ../data/rep101.csv
  -g  --go    start-up action      = data
  -h  --help  show help            = false
  -s  --seed  random number seed   = 937162211
]]
local function ask(t)
   names = table.remove(t,1)
   for c=2,#t[1] do x=t[1][c]; if x ~= "?" then lo[c]=x; hi[c]=x end end
   for r=2,#t do 
     for c=2,#t[r] do 
       x=t[r][c]; if x ~= "?" then lo[c]=math.min(x,lo[c]); hi[c]=math.max(x,hi[c]) end end
   
local function dist(t1,t2)
  for
